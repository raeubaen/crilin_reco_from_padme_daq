...
def main(arguments):
    ...
    parser.add_argument("-tdco", "--picoTDC-on", type=str, required=False, default="False", help="if the pico is ON and you wanto to match")
    ...
    evt_num = tree["Nevent"].array(library="np")#, decompression_executor=decompr_exec, interpretation_executor=interpret_exec)
    tstamp = tree["TimeStamp"].array(library="np")#, decompression_executor=decompr_exec, interpretation_executor=interpret_exec)
    ...
    reco_dict["tstamp"] = tstamp[mask_global, ...]
    reco_dict["evt_num"] = evt_num[mask_global, ...]
